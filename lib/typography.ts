...
export class AdvancedTypographyProcessor {
  ...
  private static applyBasicRules(text: string): string {
    ...
    // Закрываем открытые теги форматирования перед символом абзаца,
    // чтобы следующий параграф не наследовал начертание первых слов.
    const CLOSE_TAGS = '</strong></b></em></i>';
    text = text.replace(/\u2029/g, `${CLOSE_TAGS}\u2029`);
    // Удаляем возможные дубли закрывающих тегов (<\/b></b> и т.п.)
    text = text.replace(/(<\/(?:strong|b|em|i)>)(?=\1)/gi, '');
  ...
  // Убедимся, что символ абзаца не остаётся внутри <strong>/<b>/<em>/<i>
  outHtml = outHtml.replace(/<(strong|b|em|i)([^>]*)>([^<]*?)\u2029/gi, '<$1$2>$3</$1>\u2029');
  return outHtml;
  }
}
