...
export const RUSSIAN_TYPOGRAPHY_RULES: TypographyRule[] = [
  // Приоритет -2.2: Удаление пробелов в начале строки (но сохраняем табуляции)
  {
    ...
    pattern: /^(\t*)[ \u00A0\u2009]+/gm,
    replacement: '$1',
    description: 'Удаление пробелов в начале строки после табуляций (сохраняем табуляции для поэзии)'
  },

  // Приоритет -2.1: Удаление пробелов в начале строки (если нет табуляций)
  {
    name: 'remove_leading_spaces_no_tabs',
    priority: -2.1,
    pattern: /^[ \u00A0\u2009]+/gm,
    replacement: '',
    description: 'Удаление пробелов в начале строки (обычные пробелы, NBSP и тонкие пробелы)'
  },
